...
# 三层
    a.  普通攻击
        1). IPv4/IPv6报文头的合法性检查，如IPv4 option的检查，ipv6 EXT HDR的检查. 
        2). icmp-ping-death.
        3). tear-drop攻击.
        4). 分片攻击.
        5). icmp，ip扫描。
...
2.  四层:
    a.  普通攻击：
        1). 端口扫描，地址扫描
        2). tcp land攻击,syn-frag攻击. winnuke攻击(带外攻击).

    c. ddos攻击.
        1.  syn-flood. (syn cookie方式防护).
        2.  ack-flood. 防火墙里一般有tcp-syn-check这个选项，就是为了防护ack-flood，使得没有经过三次握手的报文无法穿越防火墙。而且防火墙一般不能回复rst，从而减少防火墙资源的使用。
...
总结：
 ...
      防火墙的防护，基本上都是采用syn-proxy/syn-cookie, 
 ...
         syn-proxy 是一个比syn－cookie重量级的技术，一般会建立session，将client的option/windowns_size/mss等相关tcp参数全部记录下来，而syn-cookie一般只记录mss（而且它也是一个近似值).
  3.  但是juniper的防火墙无法防御高级别，由确定性的攻击, 这就需要针对L7层的攻击做特定的防御。 如果防火墙后保护的是web-server, 
